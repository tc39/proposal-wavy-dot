...
It is worth noting that TypeScript has introduced postfix bang as a [non-null assertion operator](https://www.typescriptlang.org/docs/handbook/release-notes/typescript-2-0.html#non-null-assertion-operator), which conflicts with our proposed usage.  In TypeScript, `x![i]` tells the type system that `x` is not null, then evaluates as `x[i]`.  The ECMAScript [optional chaining proposal](https://github.com/TC39/proposal-optional-chaining) may mitigate the need for non-null assertion, with `x?.[i]`, `x?.p` and `x?.(...args)` syntax.
